...
export async function buildContract(sourceCode, language) {
  ...
  try {
    ...
    // Install near-sdk-js with dependencies
    console.log('📦 Installing near-sdk-js and dependencies...')
    const installStartTime = Date.now()
    
    try {
      // Install near-sdk-js with all its dependencies
      ...
      const installTime = ((Date.now() - installStartTime) / 1000).toFixed(2)
      console.log(`✓ near-sdk-js installed in ${installTime}s`)
    } catch (installError) {
      console.error('Failed to install near-sdk-js:', installError.message)
      throw new Error(`Failed to install near-sdk-js: ${installError.message}`)
    }
    ...
    const buildResult = await execAsync(`npx near-sdk-js build src/contract${ext} build/contract.wasm`, {
      cwd: tempDir,
      maxBuffer: 50 * 1024 * 1024,  // Increased buffer for large outputs
      timeout: 240000, // 4 minutes timeout for build (first build is slow)
      env: {
        ...process.env,
        NODE_ENV: 'production'
      }
    ...
    if (buildResult.stdout) {
      console.log('Build stdout:', buildResult.stdout.substring(0, 1000))
    }
    if (buildResult.stderr) {
      console.log('Build stderr:', buildResult.stderr.substring(0, 1000))
    }
